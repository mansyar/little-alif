...
│   │   ├── i18n/
│   │   │   ├── index.ts             # i18n init (typesafe-i18n)
...
Using `typesafe-i18n` — a compile-time i18n library that generates TypeScript types from translation files. Mistype a key and it won't compile.
...
app/
├── i18n/
│   ├── index.ts              # Initialization + locale detection
...
const en: Translation = {
  ...
  DASHBOARD_NO_CHILDREN: "You haven't added any children yet.",

  // Letter Toggles
  LETTERS_SHOW: 'Show to child',
  LETTERS_HIDE: 'Hide from child',

  // Child Mode
  CHILDMODE_ENABLE: 'Enable Child Mode',
  CHILDMODE_DISABLE: 'Disable Child Mode',
  CHILDMODE_ACTIVE: 'Child Mode Active',
  ...
  PROFILE_DELETE_CONFIRM: 'Are you sure? This will remove all progress.',

  // Locale
  LOCALE_SWITCH: 'Bahasa Melayu',
...
const id: Translation = {
  ...
  DASHBOARD_NO_CHILDREN: 'Belum ada anak yang ditambahkan.',

  LETTERS_SHOW: 'Tampilkan ke anak',
  LETTERS_HIDE: 'Sembunyikan dari anak',
  ...
  PROFILE_AVATAR: 'Avatar',
  PROFILE_SAVE: 'Simpan',
  PROFILE_DELETE: 'Hapus Profil',
  PROFILE_DELETE_CONFIRM: 'Yakin? Semua kemajuan akan hilang.',
...
```json
{
  "scripts": {
    "i18n": "typesafe-i18n",
    "dev": "typesafe-i18n && tanstack dev",
    "build": "typesafe-i18n && tanstack build"
  }
}
```

The generator reads the translation files and produces `i18n-types.ts`, `i18n-util.ts`, and `i18n-util.async.ts` automatically. These are checked into git (generated files are deterministic).
