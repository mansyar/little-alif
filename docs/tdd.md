...
| 26  | UX Enhancements (v1.1)              | ✅ Implemented            | [`ux-enhancements_20260608`](../conductor/archive/ux-enhancements_20260608/) |
...
│   │   ├── seed.ts                  # Seed CLI (pnpm db:seed)
│   │   ├── seed-letters.ts          # Pure seed function (no circular deps)
│   │   ├── seed-data.ts             # 28-letter master data
│   │   ├── boot.ts                  # Server startup: autoMigrate + seedLetters
...
### Server Boot Sequence (`app/db/boot.ts`)

On the Nitro server process startup, `router.tsx` imports `boot.ts` which self-executes migration + seed:
...
// Self-executes on module import (server startup only, skipped in tests)
...
The seed is idempotent — it checks for existing letter IDs before inserting.

### Seed Data (`app/db/seed-letters.ts`)

A pure function accepting a `DbClient` — no circular dependency on `getDb()`:
...
The `seed.ts` CLI (`pnpm db:seed`) reuses `seedLetters()` — it calls `getDb()` first, then delegates to the pure function.
